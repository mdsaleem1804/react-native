...
export default class OwnerRegistration extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      input_user_id: '',
      ...
      if (user_phone) {
        if (user_password) {
          db.transaction(function(tx) {
            tx.executeSql(
              'INSERT INTO table_owner (owner_name, owner_phone, owner_password) VALUES (?,?,?)',
    ...
    if (user_name) {
      if (user_phone) {
        if (user_owner_id) {
          db.transaction(tx => {
            tx.executeSql(
              'UPDATE table_owner set owner_name=?, owner_phone=? , owner_password=? where owner_id=?',
              [user_name, user_phone, user_password, input_user_id],
              (tx, results) => {
                console.log('Results', results.rowsAffected);
                if (results.rowsAffected > 0) {
                  Alert.alert(
  ...
  deleteDriver = () => {
    ...
    db.transaction(tx => {
      tx.executeSql(
        'DELETE FROM  table_owner where owner_id=?',
        [input_user_id],
        (tx, results) => {
          console.log('Results', results.rowsAffected);
          if (results.rowsAffected > 0) {
            ...
            alert('Please insert a valid Owner Id');
          }
        },
      );
    });
  ...
  render() {
    const {navigation} = this.props;
    const itemId = navigation.getParam('OwnerId', '');
    return (
